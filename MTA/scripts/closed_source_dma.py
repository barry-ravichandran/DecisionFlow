...
def load_system_message(alignment=None,
                        system_messages_path=cot_system_messages_path):
...
class ClosedSingleDMA:
    ...
    def respond_to_dialog(self, dialog, prefix=None):
        ...
        inference_pair['output'] = statement
        return statement, inference_pair

    def aligned_decision_maker(self, question, choices, target_kdmas, structure, attribute, alignment):
        inference_pairs = []
        ...
        try:
            reasoning, answer_idx, parse_method = ClosedSingleDMA.parse_generated_output(high_response, len(choices))
            good_parse = True
        except RuntimeError as e:
            pass

        if not good_parse:
            reasoning, answer_idx, parse_method = (
                ClosedSingleDMA.bert_similarity_parse(
                    high_response, shuffled_choices
                )
            )

        print('CHOSEN ANSWER IDX', answer_idx, shuffled_choices)
        assert answer_idx is not None, f'Failed to parse answer index from generated output'
    ...
    def run_aligned_decision_maker_with_voting(
        ...
        try:
            choice_scores = ClosedSingleDMA.calculate_votes(responses, choices)
        except Exception as e:
